...
class ProfilePhoto < ActiveRecord::Base
    ...
    def validate
        ...
        if !self.draft && (self.image.columns != WIDTH || self.image.rows != HEIGHT)
            errors.add(:data, _("Failed to convert image to the correct size: at %{cols}x%{rows}, need %{width}x%{height}" % { :cols => self.image.columns, :rows => self.image.rows, :width => WIDTH, :height => HEIGHT }))
        end
